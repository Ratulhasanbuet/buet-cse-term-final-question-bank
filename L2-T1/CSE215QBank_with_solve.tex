...
\begin{tcolorbox}[enhanced,colback=cA!5,colframe=cA,boxrule=1.5pt,arc=6pt,
  ...
  {\fontsize{22}{28}\selectfont\bfseries\textcolor{cA}{Database Question Bank}}\\[8pt]
...
\vspace{1em}
\noindent\textbf{Exam Tip:} When differentiating the two architectures, always emphasize where the \textbf{Business Logic} resides. In 2-tier, it is bundled with the client; in 3-tier, it is isolated in the application server.
...
\begin{tikzpicture}[
    ...
    % Level Labels
    \node[leveltext] at (-4, 4) {1. View (External) Level};
    \node[leveltext] at (-4, 1.5) {2. Logical (Conceptual) Level};
    \node[leveltext] at (-4, -1) {3. Physical (Internal) Level};
    \node[leveltext] at (-4, -3.5) {Physical Storage};
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} A key concept intrinsically linked to data abstraction is \textbf{Data Independence}. 
\begin{itemize}
    \item \textit{Logical Data Independence:} Changing the logical schema (e.g., adding a new column to a table) without rewriting application programs at the View level.
    \item \textit{Physical Data Independence:} Changing the physical schema (e.g., migrating from HDD to SSD, or switching from a Hash index to a B+ Tree) without altering the Logical schema.
\end{itemize}
\end{tcolorbox}
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} While derived attributes save storage and guarantee data freshness, they consume CPU cycles during execution. In highly transactional systems where complex derivation formulas are queried millions of times, developers might choose to store them physically and manage updates via database \textbf{Triggers} to optimize read performance.
...
\vspace{1em}
\noindent\textbf{Exam Tip:} Always underline the primary key attribute in your ER diagrams. If an exam question asks for attributes, ensure you include a mix of key, simple, multivalued, or derived attributes to demonstrate comprehensive knowledge.
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} In SQL, the standard \texttt{UNION}, \texttt{INTERSECT}, and \texttt{EXCEPT} (or \texttt{MINUS}) operators perform \textbf{Set} operations by default (they implicitly remove duplicates). To instruct SQL to use \textbf{Bag} operations, you must append the keyword \texttt{ALL}, such as \texttt{UNION ALL}, \texttt{INTERSECT ALL}, or \texttt{EXCEPT ALL}.
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When answering questions about DBMS properties, always explicitly differentiate between the \textbf{general structural properties} (like Data Independence and Abstraction) and the \textbf{transactional properties} (ACID). Explicitly listing ACID guarantees full marks for the transactional aspect of the DBMS.
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When describing the architecture, emphasize that the \textbf{Storage Manager} is the critical bridge. The Query Processor does not touch the disk directly; it must request data via the Storage Manager's Buffer and File Managers.
...
\begin{tikzpicture}[
    ...
    \node[file] (f1) at (-5.5, 1.5) {Employees.dat \\ \normalfont\scriptsize (Stored locally)};
    \node[file] (f2) at (-2.5, 1.5) {Staff.txt \\ \normalfont\scriptsize (Stored locally)};
    ...
    % --- Separator ---
    \draw[loosely dashed, gray!80] (0, -2.5) -- (0, 6);
\end{tikzpicture}
\end{center}
\vspace{1.5em}
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When answering this question, explicitly group your points. The most critical contrasts examiners look for are \textbf{Data Independence}, the elimination of \textbf{Redundancy}, and the presence of \textbf{ACID transactional guarantees}. Mentioning ACID guarantees full marks for the operational properties of a DBMS.
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When drawing the block diagram in an exam, the most crucial relationship to establish is that the \textbf{Query Processor} does \textit{not} interact with the disk directly. All data requests must pass through the \textbf{Storage Manager} (specifically the Buffer Manager and File Manager).
...
\begin{tikzpicture}[
    ...
    % Data Node
    \node[box, minimum height=2.2cm, font=\ttfamily] (data) at (0, -0.5) {
        \begin{tabular}{llc}
    ...
    \node[font=\bfseries, anchor=south west] at (-3.75, 0.6) {Data / Instance (Records / Extension)};
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} To guarantee full marks, always map the two concepts together: \textbf{DDL} is the language used to define the \textbf{Schema}, whereas \textbf{DML} is the language used to manage the \textbf{Data (Instance)}.
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When discussing "Multiple relations in one file", explicitly use the term \textbf{Multitable Clustering}. This keyword demonstrates a deep understanding of physical storage optimization techniques used by modern systems like Oracle (Table Clusters) to optimize join operations.
...
\begin{enumerate}\item Differentiate between the Nested-Loop Join algorithm and the Block Nested-Loop Join algorithm.
\mk{5} \yr{CSE 215 | L-2/T-1 | 2024--25}
