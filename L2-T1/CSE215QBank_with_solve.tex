...
\begin{tikzpicture}[
    thick,
    cell/.style={draw, rectangle, minimum height=0.7cm, align=center, fill=white, font=\small},
    head/.style={draw, rectangle, minimum height=0.7cm, align=center, fill=black!10, font=\small\bfseries},
    arrow/.style={->, >=latex, thick, rounded corners=4pt, blue!70!black}
]
    % Employees Table Setup
    \node[font=\bfseries] at (-2, 2) {Employees Table (Source)};
    \node[head, minimum width=1.5cm] (eid) at (-4.25, 1.2) {Emp\_ID};
    \node[head, minimum width=2.5cm] (enm) at (-2.25, 1.2) {Name};
    \node[head, minimum width=2.5cm] (ewf) at (0.25, 1.2) {Works\_For};
    
    \node[cell, minimum width=1.5cm] (r1_id) at (-4.25, 0.5) {101};
    \node[cell, minimum width=2.5cm] (r1_nm) at (-2.25, 0.5) {Alice};
    \node[cell, minimum width=2.5cm, fill=blue!5] (r1_wf) at (0.25, 0.5) {\itshape 0x9A4F...}; % OID value

    % Departments Table Setup
    \node[font=\bfseries] at (4.5, 2) {Departments Table (Target)};
    \node[head, minimum width=2.5cm] (doid) at (3.5, 1.2) {OID (System)};
    \node[head, minimum width=1.5cm] (dno) at (5.5, 1.2) {Dept\_No};
    \node[head, minimum width=2cm] (dnm) at (7.25, 1.2) {Dept\_Name};

    \node[cell, minimum width=2.5cm, fill=blue!5] (r2_oid) at (3.5, 0.5) {\itshape 0x9A4F...};
    \node[cell, minimum width=1.5cm] (r2_no) at (5.5, 0.5) {10};
    \node[cell, minimum width=2cm] (r2_nm) at (7.25, 0.5) {Research};

    % Reference Arrow
    \draw[arrow] (r1_wf.south) -- +(0,-0.5) -| (r2_oid.south) node[pos=0.25, above, font=\footnotesize\bfseries, text=black] {Dereference (Pointer Traversal)};
...
\begin{tikzpicture}[x=1cm, y=1cm, thick]
    % Styles
    \tikzset{
        tablebox/.style={draw, rectangle, minimum height=0.9cm, align=center, fill=white},
        tablename/.style={draw, rectangle, minimum height=0.9cm, align=center, fill=black!10, font=\bfseries}
    }

    % EMPLOYEE Table
    \node[tablename, minimum width=2.5cm] at (1.25, 2.5) {EMPLOYEE};
    \node[tablebox, minimum width=2.5cm, font=\bfseries] (pk_emp) at (3.75, 2.5) {\underline{Emp\_ID}};
    \node[tablebox, minimum width=2.5cm] at (6.25, 2.5) {Name};

    % DEPENDENT Table
    \node[tablename, minimum width=2.5cm] at (1.25, 0.5) {DEPENDENT};
    \node[tablebox, minimum width=2.5cm, font=\bfseries, align=center] (fk_dep) at (3.75, 0.5) {\underline{Emp\_ID} \\[-0.5ex] \normalfont\scriptsize (FK)};
    \node[tablebox, minimum width=2.5cm, font=\bfseries] at (6.25, 0.5) {\underline{Dep\_Name}};
    \node[tablebox, minimum width=2.5cm] at (8.75, 0.5) {Relation};

    % Foreign Key Arrow
    \draw[->, >=latex, rounded corners=3pt] (fk_dep.north) -- (3.75, 1.5) -| (pk_emp.south);

    % Note
    \node[anchor=west, font=\footnotesize\itshape] at (0, -0.7) {* Note: The primary key of DEPENDENT is composite: (Emp\_ID, Dep\_Name).};
...
\hrule
\vspace{0.5em}
\noindent\textbf{Exam Tip:} When mapping a weak entity, you do \textbf{not} create a separate relation (table) for the identifying relationship (e.g., the "Has" diamond). The relationship is implicitly managed by the foreign key placed inside the weak entity's table.
...
\begin{tikzpicture}[
    ...
    \node[attribute] (rank) at (-1, 0) {Rank};
    \node[attribute] (position) at (9, 0) {Position};
    \draw (faculty) -- (rank);
    \draw (staff) -- (position);
...
\begin{tikzpicture}[
    ...
    % Foreign Key Arrow
    \draw[->, thick, rounded corners] (t2c1.south) -- +(0,-0.5) -| (t1c1.south);
...
\begin{tikzpicture}[
    ...
    \draw (ISSUES) -- (BOOK_COPY) node[midway, right, font=\bfseries] {N};
    ...
    \node[attribute] (tc_desg) at (-1, -7) {Designation};
...
\begin{tikzpicture}[
    ...
    entity/.style={draw, rectangle, rounded corners, minimum width=2.5cm, minimum height=1cm, align=center, fill=white, font=\bfseries},
    attribute/.style={draw, ellipse, minimum width=2cm, minimum height=0.7cm, align=center, fill=white, font=\footnotesize},
    multi_attribute/.style={draw, double, double distance=1.5pt, ellipse, minimum width=2cm, minimum height=0.7cm, align=center, fill=white, font=\footnotesize}
]
    % ER Diagram
    \node[font=\bfseries] at (0, 2) {ER Diagram};
    
    \node[entity] (emp) at (0, 0) {EMPLOYEE};
    \node[attribute] (eid) at (-3, 0) {\underline{Emp\_ID}};
    \node[multi_attribute] (phone) at (3, 0) {Phone};
    ...
    \node[align=left] at (0, -3.5) {
        ...
        \begin{tabular}{|c|}
        \hline
        \cellcolor{gray!20}\textbf{\underline{Emp\_ID}} \\
        \hline
        \end{tabular}
        ...
        \begin{tabular}{|c|c|}
        \hline
        \cellcolor{gray!20}\textbf{\underline{Emp\_ID}} (FK) & \cellcolor{gray!20}\textbf{\underline{Phone}} \\
        \hline
        \end{tabular}
    };
    
    % Foreign key constraint arrow (using tikz overlays conceptually within standard coords)
    \draw[->, rounded corners] (-1.3, -3.8) -- (-2.5, -3.8) -- (-2.5, -2.85) -- (-1, -2.85);
...
\begin{tikzpicture}[
    ...
    \node[attribute] (title) at (-2.2, -0.2) {Title};
    \node[attribute] (pages) at (-1.5, -1.5) {Num\_Pages};
    \node[attribute] (price) at (1.5, -1.5) {Price};
    \node[attribute] (class) at (2.4, -0.2) {Classification};
